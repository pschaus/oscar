 ...
 */
object Crew extends CPModel with App  {
    //
    // data
    // 
    var num_to_show = 1
    ...
    // variables
    //
    val crew = Array.fill(num_flights,num_persons)(CPIntVar(0 to 1))
    ...
    //minimize(num_working) 
      ...
      for(f <- 0 until num_flights - 2) {
        for(p <- PERSONS) {
         add(crew(f)(p) + crew(f+1)(p) + crew(f+2)(p) <= 1)
        }
      }
      ...
      /*
      for(p <- PERSONS) {
       add(sum(for{f <- FLIGHTS} yield(crew(f)(p))) >= 2)
      }
      */
      // extra constraint: No one should work more than 4 flights
      /*
      for(p <- PERSONS) {
       add(sum(for{f <- FLIGHTS} yield(crew(f)(p))) <= 4)
      }
      */
     search{
