...
*/
object Hidato extends CPModel with App  {
    //
    // data
    //
    // solution:
    ...
    // Problem 156 (Master)
    // (This seems to be harder tothan the 12x12 prolem 188 below...)
    ...
    var backtracks = 0
     ...
     add(allDifferent(x), Strong)
      for(k <- 1 until n*n-1) {
        val i = CPIntVar(0 to n-1)
        val j = CPIntVar(0 to n-1)
        val a = CPIntVar(-1 to 1)
        val b = CPIntVar(-1 to 1)
        val ix1 = i*n+j
        val ix2 = (i+a)*n + j+b
