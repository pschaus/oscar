 ...
 *
 */
object Rogo2 extends CPModel with App  {
  val W = 0;
  val B = -1;
  // Global variables
  ...
  def readFile(problem_file: String): Unit = {
    ...
    // variables
    //
    val path       = Array.fill(max_steps)(CPIntVar(0 to rows*cols-1))
    val points     = Array.fill(max_steps)(CPIntVar(0 to best))
    val sum_points = CPIntVar(0 to best)
    //
    // constraints
    //
    var numSols = 0
   maximize(sum_points) 
     ...
     add(allDifferent(path), Strong)
      // valid connections
      for(s <- STEPS1) {
       add(table(Array(path(s), path(s+1)), valid_connections))
      }
      // around the corner
     add(table(Array(path(max_steps-1), path(0)), valid_connections))
      // Symmetry breaking
