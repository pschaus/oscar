...
final class ArrayHeapDouble(maxSize: Int) {

  private[this] val values: Array[Int] = new Array[Int](maxSize + 1)
  private[this] val keys: Array[Double] = new Array[Double](maxSize + 1)
  ...
  def enqueue(key: Double, value: Int): Unit = {
  ...
  def remove(key: Double, value: Int): Boolean = {
    if (heapSize == 0) false
    else {
  ...
  def changeKey(oldKey: Double, newKey: Double, value: Int): Boolean = {
    if (heapSize == 0) false
    else {
  ...
  @inline @tailrec private def search(id: Int, key: Double, value: Int): Int = {
    if (id > heapSize) id
