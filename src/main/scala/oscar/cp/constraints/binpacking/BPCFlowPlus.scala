...
/**
  ...
  */
class BPCFlowPlus(items: Array[CPIntVar], sizes: Array[Int], val l: Array[CPIntVar], val c: Array[CPIntVar]) extends GCCVarAC(items, 0, c) {
  ...
  def checkBinLoad(bin: Int): Unit = {
    ...
    //ADDED PLUS
    //We are now at min card, min load.
    //An unmatched item cannot be added if, when added, the weight is > l(bin).max
    up(bin) = c(bin).max
    fillReachableFrom(bin, false)
    var obj = 0
    while(obj != items.length) {
      if(items(obj).hasValue(bin) && varMatch(obj) != bin) {
        if(reachableFrom(obj) == NONE)
          items(obj).removeValue(bin)
        else if((load - reachableFrom(obj) + sizes(obj)) > l(bin).max) {
          items(obj).removeValue(bin)
        }
      }
      obj += 1
    ...
    //ADDED PLUS
    //We are now at max card, max load.
    //An unmatched item cannot be added if, when added, the weight is < l(bin).min
    low(bin) = c(bin).min
    fillReachableFrom(bin, true)
    obj = 0
    while(obj != items.length) {
      if(items(obj).hasValue(bin) && varMatch(obj) != bin) {
        if(reachableFrom(obj) == NONE || (load - reachableFrom(obj) + sizes(obj)) < l(bin).min) {
          items(obj).removeValue(bin)
        }
      }
      obj += 1
