...
class BasicSmartDiagram(transactionByVar: Array[Array[(Int, BasicSmartElement, Int)]], nbNodesByVar: Array[Int]) extends Diagram {
  val nbLevel = transactionByVar.length

  def countCategory = {
    val count = Array.fill(SmartElement.listBasicSmartElement.length)(0)
    for {
      level <- transactionByVar
      (_, bs, _) <- level
    } {
      count(bs.classId) += 1
      count
  ...
  def countCategoryLvl(lvl: Int) = {
    val count = Array.fill(SmartElement.listBasicSmartElement.length)(0)
    for {
      (_, bs, _) <- transactionByVar(lvl)
    } {
      count(bs.classId) += 1
    }
    count
  ...
  def getHashSet(x: Array[CPIntVar]): mutable.Set[String] = {
    ...
    for (lvl <- 0 until nbLevel) {
      ...
      for ((from, values, to) <- transactionByVar(lvl)) {
        values.foreach(x(lvl), value => {
          if (lvl == 0)
  ...
  def filterDiagram(x: Array[CPIntVar]): Diagram = {
    ...
    val transitionDownPass = Array.fill(nbLevel)(new ArrayBuffer[(Int, BasicSmartElement, Int)]())
    var lvl = 0
    while (lvl < nbLevel) {
      ...
      while (n > 0) {
        ...
        if (parents.contains(n)) {
          val filtered = parents(n).filter(t => !removedNode(lvl).contains(t._1) && t._2.isValid(x(lvl)))
          if (filtered.isEmpty) {
    ...
    // 2) upward pass
    val transitionUpPass = Array.fill(nbLevel)(new ArrayBuffer[(Int, BasicSmartElement, Int)]())
    lvl = nbLevel
    while (lvl > 0) {
      lvl -= 1
      ...
      while (n > 0) {
        ...
        if (!removedNode(lvl).contains(n)) {
          if (child.contains(n)) {
            val filtered = child(n).filter(t => !removedNode(lvl + 1).contains(t._3) && t._2.isValid(x(lvl)))
            if (filtered.isEmpty)
    ...
    new BasicSmartDiagram(transitionFinal, nbNodesByVar.clone)
  }

  def applyOffset(offsets: Array[Int]): Diagram = {
    val transition = Array.tabulate(transactionByVar.length) {
      lvl =>
        Array.tabulate(transactionByVar(lvl).length) {
          i => (transactionByVar(lvl)(i)._1, transactionByVar(lvl)(i)._2.applyOffset(offsets(lvl)).asInstanceOf[BasicSmartElement], transactionByVar(lvl)(i)._3)
        }
    }
    new BasicSmartDiagram(transition, nbNodesByVar.clone())
  }

  def decompressToTable(x: Array[CPIntVar]): Table = {
    var buff: Array[Array[Array[BasicSmartElement]]] = Array(Array(new Array[BasicSmartElement](nbLevel)))
    for (lvl <- 0 until nbLevel) {
      val newArray = Array.fill(nbNodesByVar(lvl + 1))(new ArrayBuffer[Array[BasicSmartElement]])
  ...
  def reduceTree_sMDD: Diagram = {
    ...
    val smdd = rTDownward(mid - 1).asInstanceOf[BasicSmartDiagram]
  ...
  private def rTUpward(from: Int): Diagram = {
    val resultTransition = new Array[Array[(Int, BasicSmartElement, Int)]](nbLevel)
    ...
    while (lvl > 0) {
      ...
      if (lvl < from) {
        ...
        val sortedChild = Array.tabulate(nbNodesByVar(lvl))(n => child(n).sortBy(i => (SmartElement.getSortingTuple(transForNext(i)._2), transForNext(i)._3)).toArray)
        ...
        resultTransition(lvl) = new Array[(Int, BasicSmartElement, Int)](nbTrans)
        var next = 0
        var i = sizeSparseSet
        while (i > 0) {
          i -= 1
    ...
    new BasicSmartDiagram(resultTransition, nodes)
  }
  ...
  private def rTDownward(from: Int): Diagram = {
    ...
    while (lvl < nbLevel) {
      if (lvl > from) {
        ...
        // 2) sort child for each node by (value,resultnode)
        val sortedParent = Array.tabulate(nbNodesByVar(lvl + 1))(n => parents(n).sortBy(i => (SmartElement.getSortingTuple(transForNext(i)._2), transForNext(i)._1)).toArray)
        // 3) compute hash for each node
  ...
  private def hashNode(transition: Array[(Int, BasicSmartElement, Int)], id: Array[Int], isUpward: Boolean): String = {
    if (id.length > 0) {
      ...
      while (i < id.length) {
        buff.append(",")
        buff.append(transition(id(i))._2.toString)
        buff.append("-")
        if (isUpward)
