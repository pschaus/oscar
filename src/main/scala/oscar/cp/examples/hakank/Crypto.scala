...
*/
object Crypto extends CPModel with App  {
    //
    // data
    //
    val num_letters = 26
    ...
    // variables
    //
    val LD = Array.fill(num_letters)(CPIntVar(1 to num_letters))
    ...
    var numSols = 0
     ...
     add(allDifferent(LD), Strong)
     add(            B + A + L + L + E + T === BALLET)
     add(                C + E + L + L + O === CELLO)
     add(        C + O + N + C + E + R + T === CONCERT)
     add(                F + L + U + T + E === FLUTE)
     add(                F + U + G + U + E === FUGUE)
     add(                    G + L + E + E === GLEE)
     add(                    J + A + Z + Z === JAZZ)
     ...
     add(                O + P + E + R + A === OPERA)
     add(                P + O + L + K + A === POLKA)
     add(        Q + U + A + R + T + E + T === QUARTET)
     add(S + A + X + O + P + H + O + N + E === SAXOPHONE)
     add(                S + C + A + L + E === SCALE)
     add(                    S + O + L + O === SOLO)
     add(                    S + O + N + G === SONG)
     add(        S + O + P + R + A + N + O === SOPRANO)
     add(                T + H + E + M + E === THEME)
     add(            V + I + O + L + I + N === VIOLIN)
     add(                W + A + L + T + Z === WALTZ)
    search{
      binaryMaxDegree(LD)
    }
