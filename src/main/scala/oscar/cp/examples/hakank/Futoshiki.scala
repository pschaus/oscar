...
*/
object Futoshiki extends CPModel with App  {
    //
    // data
    //
   //
    ...
    // variables
    //
    val field = Array.fill(size)(Array.fill(size)(CPIntVar(1 to size)))
    //
    // constraints
    //
    var numSols = 0
      ...
      for(row <- RANGE) {
       add(allDifferent(for(col <- RANGE) yield field(row)(col)))
      }
      // all columns have to be different
      for(col <- RANGE) {
       add(allDifferent(for(row <- RANGE) yield field(row)(col)))
      }
      // all < constraints are satisfied
      // Also: make 0-based
      for(i <- NUMQD) {
       add(field(lt(i)(0)-1)(lt(i)(1)-1) <
