 ...
 * """
 *
 * Here wethe problem from the Wikipedia page, also shown here
 ...
 *
 */
object KillerSudoku extends CPModel with App  {
  /**
  ...
  def calc(
   ...
   add(sum(for{i <- 0 until len} yield x(cc(i*2)-1)(cc(i*2+1)-1)) === res)
    ...
    // Decision variables
    // 
    val x = Array.fill(n,n)(CPIntVar(0 to 9))
    ...
    var numSols = 0
      ...
      // blocks
      for(i <- CELLS; j <- CELLS) {
       add(allDifferent(  (for{ r <- i*cell_size until i*cell_size+cell_size;
