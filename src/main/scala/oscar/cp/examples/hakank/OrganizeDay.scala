...
*/
object OrganizeDay extends CPModel with App  {
  ...
  def noOverlap(s1: CPIntVar, d1: Int,
    ...
    // variables
    //
    val begins = Array.fill(n)(CPIntVar(begin to end))
    val ends   = Array.fill(n)(CPIntVar(begin to end))
    //
    // constraints
    //
    var numSols = 0
      ...
      for(i <- tasks;
          j <- tasks if i < j) {
       add(noOverlap(begins(i), durations(i),
                         begins(j), durations(j)))
      }
      // specific constraints
      for(t <- 0 until before_tasks.length) {
       add(ends(before_tasks(t)(0)) <= begins(before_tasks(t)(1)));
      }
     add(begins(work) >= 11)
    search{
